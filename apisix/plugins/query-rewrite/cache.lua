...
local function use_fallback(conf)
    if conf.backend == "local" then
        return true
    end

    return shared_dict():get(breaker_key(conf)) ~= nil
end

local function mark_backend_failure(conf, err)
    shared_dict():set(breaker_key(conf), true, conf.fallback_ttl)
    core.log.warn("query-rewrite cache backend unavailable: ", err,
                  "; using local memory for ", conf.fallback_ttl, " seconds")
end

local function local_get(key)
    return shared_dict():get(local_key(key))
end

local function local_set(key, value, ttl)
    local ok, err = shared_dict():set(local_key(key), value, ttl)
    if not ok then
...
local function cluster_get(conf, key)
    local red, err = rediscluster.new(conf, "query-rewrite-redis-cluster-slot-lock")
    if not red then
        return nil, err
    end
...
local function backend_get(conf, key)
    if use_fallback(conf) then
        return local_get(key), nil, "local-fallback"
    end

    if conf.backend == "local" then
        return local_get(key), nil, "local"
    end
...
local function backend_set(conf, key, value, ttl)
    if use_fallback(conf) then
        return local_set(key, value, conf.fallback_ttl)
    end
...
local function request_is_cacheable(conf, headers)
    ...
    if has_directive(request_cache_control, "no%-store") or
       ...
       headers["pragma"] and lower(headers["pragma"]):find("no%-cache", 1, false) then
...
function _M.body_filter(conf, ctx)
    ...
    if not ok then
        core.log.warn("failed to schedule query cache store: ", err)
    end
end
